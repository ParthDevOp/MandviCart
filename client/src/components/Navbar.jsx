...
import { 
    ...
    Phone, LayoutDashboard, Package, Info, BellOff, Activity, ChevronRight, Truck
} from 'lucide-react';
...
const Navbar = () => {
    ...
    const handleMouseEnter = (popupName) => {
        ...
        if (popupName === 'order') {
            ...
            setShowCartHover(false); // Close Cart
            setShowNotif(false);     // Close Notifs
            setIsSearchFocused(false); // Close Search
        } else if (popupName === 'cart') {
            setShowCartHover(true);
            setShowOrderHover(false); // Close Orders
            setShowNotif(false);      // Close Notifs
            setIsSearchFocused(false); // Close Search
        }
    };
    ...
    return (
        <>
        <nav className={`flex items-center justify-between px-4 md:px-12 lg:px-24 py-3 transition-all duration-300 fixed w-full z-[999] top-0 font-outfit
            ...
            {/* 🟢 DESKTOP NAVIGATION */}
            {allowSurfing && (
                <div className="hidden lg:flex items-center justify-center gap-8 font-bold text-slate-500 text-[15px] flex-1 px-8">
            ...
            <div className="flex items-center gap-3 md:gap-5 shrink-0">
                ...
                {allowSurfing && (
                    <div ref={searchRef} className="hidden md:block relative z-50">
                        <div className={`flex items-center text-sm gap-2.5 px-4 py-2.5 rounded-2xl w-64 lg:w-80 transition-all duration-200 border
                            ...
                            <input 
                                ...
                                onFocus={handleSearchFocus} /* 🟢 Applied Mutual Exclusion Here */
                                ...
                                type="text" placeholder="Search fresh groceries..." 
                            />
                            {localSearch && (
                        ...
                        {isSearchFocused && localSearch.length > 0 && (
                            <div className="absolute top-[110%] left-0 w-full bg-white/90 backdrop-blur-xl shadow-[0_20px_50px_-10px_rgba(0,0,0,0.15)] border border-white/50 rounded-2xl overflow-hidden animate-fade-in-up">
                ...
                <div className="flex items-center gap-2 md:gap-3">
                            ...
                            <AnimatePresence>
                                {showNotif && (
                                    <motion.div 
                                        ...
                                        className="absolute right-0 mt-3 w-80 bg-white/90 backdrop-blur-xl shadow-[0_20px_50px_-10px_rgba(0,0,0,0.15)] border border-white/50 rounded-2xl overflow-hidden z-50 origin-top-right"
                                    >
                    ...
                    {/* 🟢 FUTURISTIC ORDERS HOVER (Customer Only) */}
                    {isCustomer && (
                        <div 
                            className="relative hidden sm:block"
                            onMouseEnter={() => handleMouseEnter('order')} /* 🟢 Applied Mutual Exclusion Here */
                            ...
                            <AnimatePresence>
                                {showOrderHover && (
                                    <motion.div 
                                        ...
                                        className="absolute top-[120%] right-0 w-72 bg-white/80 backdrop-blur-2xl shadow-[0_30px_60px_-15px_rgba(0,0,0,0.3)] border border-white/60 rounded-[2rem] p-5 z-50 origin-top-right"
                                    >
                                        {liveOrder ? (
                                            <div>
                    ...
                    {allowSurfing && !isStaff && (
                        <div 
                            className="relative hidden sm:block"
                            onMouseEnter={() => handleMouseEnter('cart')} /* 🟢 Applied Mutual Exclusion Here */
                        ...
                        >
                            ...
                            <AnimatePresence>
                                {showCartHover && (
                                    <motion.div 
                                        initial={{ opacity: 0, y: 15, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} transition={{ type: "spring", stiffness: 400, damping: 25 }}
                                        className="absolute top-[120%] right-0 w-64 bg-white/80 backdrop-blur-2xl shadow-[0_30px_60px_-15px_rgba(0,0,0,0.3)] border border-white/60 rounded-[2rem] p-5 z-50 origin-top-right"
                                    >
                    ...
                    {/* 🟢 MOBILE MENU TOGGLE */}
                    <button onClick={() => setOpen(true)} className="md:hidden w-10 h-10 flex items-center justify-center text-slate-600 bg-slate-100 rounded-full active:scale-95"><Menu size={20} /></button>
                </div>
            </div>
        </nav>
            ...
            <div className="flex-1 overflow-y-auto px-4 py-6 flex flex-col gap-2 bg-white">
                ...
                {allowSurfing && (
                    <div className="space-y-1.5">
                        ...
                        {isCustomer && (
                            <NavLink onClick={()=>setOpen(false)} to='/my-orders' className={({isActive}) => `py-3.5 px-4 rounded-xl font-bold flex items-center gap-3 transition-colors ${isActive ? 'bg-indigo-50 text-indigo-700' : 'hover:bg-slate-50 text-slate-600'}`}>
                                {({isActive}) => (<><Package size={18} className={isActive ? 'text-indigo-500' : 'text-slate-400'}/> My Orders</>)}
                            </NavLink>
                        )}
